...
import 'features/dashboard/screens/phase_one_preview.dart';
import 'features/dashboard/screens/phase_five_preview.dart';
import 'features/dashboard/screens/phase_six_preview.dart';
...
Future<void> main() async {
  ...
  // A deterministic, dependency-free entry point used only for visual QA.
  // Production builds never enable this flag and keep the normal startup flow.
  if (const bool.fromEnvironment('SCREENSHOT_MODE')) {
    if (const String.fromEnvironment('SCREENSHOT_PAGE') == 'phase_one_shell') {
      final homeDesign =
          int.tryParse(Uri.base.queryParameters['design'] ?? '0') ?? 0;
      if (homeDesign > 0) {
        if (homeDesign == 4) {
          runApp(ProfileDashboardPreview(
            isDark: Uri.base.queryParameters['theme'] == 'dark',
          ));
          return;
        }
        if (homeDesign == 5) {
          runApp(CheckoutPhasePreview(
            isDark: Uri.base.queryParameters['theme'] == 'dark',
          ));
          return;
        }
        if (homeDesign == 6) {
          runApp(PhaseFivePreview(
            isDark: Uri.base.queryParameters['theme'] == 'dark',
            page: Uri.base.queryParameters['page'] ?? 'cart',
          ));
          return;
        }
        if (homeDesign == 7) {
        ...
        runApp(HomeDesignPreview(
          design: homeDesign,
          isDark: Uri.base.queryParameters['theme'] == 'dark',
        ));
        return;
      }
      runApp(PhaseOnePreview(
        isDark: Uri.base.queryParameters['theme'] == 'dark' ||
            const bool.fromEnvironment('SCREENSHOT_DARK_MODE'),
        headerStyle:
            int.tryParse(Uri.base.queryParameters['header'] ?? '1') ?? 1,
      ));
      return;
    }
    runApp(
  ...
  runApp(MultiProvider(
    providers: [
      ...
      ChangeNotifierProvider(
          create: (context) => di.sl<CustomerOrderInsuranceController>()),
      ChangeNotifierProvider(
...
class MyApp extends StatefulWidget {
  ...
  final String? route;
  const MyApp({super.key, required this.body, this.route});
    ...
    if (widget.route != null) {
      ...
      if (mounted) {
        Provider.of<SplashController>(context, listen: false)
            .initConfig(context, (_) {}, (ConfigModel? configModel) async {
          if (configModel != null) {
