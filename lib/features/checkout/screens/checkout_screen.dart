...
class CheckoutScreenState extends State<CheckoutScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final physicalCartGroupIds = widget.cartList
        .where((cart) =>
            cart.productType == 'physical' && (cart.isChecked ?? false))
    ...
    if (_quoteKey != quoteKey) {
      ...
      WidgetsBinding.instance.addPostFrameCallback((_) async {
        if (!mounted) return;
        await checkout.getOrderInsuranceQuote(
            couponCode: coupon.discount != null && coupon.discount != 0
                ? coupon.couponCode
                : '');
        if (mounted && _quoteKey == quoteKey)
          setState(() => _quoteReadyKey =
              checkout.orderInsuranceQuote == null ? null : quoteKey);
      });
    }
      ...
      bottomNavigationBar:
        ...
        return Consumer<CheckoutController>(
          ...
          return Consumer<CouponController>(
            ...
            if (orderProvider.orderInsuranceQuote?.postPurchaseEnabled ==
                true) {
              _tax = 0;
            } else if (splashController.configModel?.systemTaxIncludeStatus !=
                1) {
                ...
                child: ListView(
                  ...
                  children: [
                    if (!orderProvider.insuranceQuoteLoading &&
                      ...
                      TextButton.icon(
                          onPressed: () => setState(() => _quoteKey = null),
                          icon: const Icon(Icons.refresh),
                          label: Text(
                              getTranslated('checkout_quote_retry', context) ??
                                  '')),
                    Padding(
                          ...
                          return Column(
                            ...
                            children: [
                              ...
                              AmountWidget(
                                ...
                                isTitleBlack: true,
                                title:
                                    '${getTranslated('total_payable', context)} ${checkoutController.orderInsuranceQuote?.postPurchaseEnabled != true && splashController.configModel?.systemTaxIncludeStatus == 1 ? getTranslated('inc_vat_tax', context) : ''}',
                                amount: PriceConverter.convertPrice(
                                  context,
                                  checkoutController.orderInsuranceQuote
                                          ?.firstPaymentAmount ??
                                      (_order +
                                          selectedShippingFee -
                                          (_referralDiscount ?? 0) -
