                    ...
                    ? SingleChildScrollView(
                        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          if (hasWallet) ListTile(
                            ...
                            subtitle: Text('${getTranslated('purchase_wallet_only_notice', context) ?? ''}\n${PriceConverter.convertPrice(context, context.watch<ProfileController>().userInfoModel?.walletBalance ?? 0)}'),
                            trailing: Row(mainAxisSize: MainAxisSize.min, children: [
                              TextButton(
                                onPressed: () => Navigator.of(context).push(MaterialPageRoute(builder: (_) => const CustomerWalletScreen(initialWallet: 'purchase'))),
                                child: Text(getTranslated('wallet_make_deposit', context) ?? ''),
                              ),
                              checkoutController.isWalletChecked ? Icon(Icons.check_circle, color: Theme.of(context).primaryColor) : const Icon(Icons.circle_outlined),
                            ]),
