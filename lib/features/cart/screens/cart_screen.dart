...
class CartScreenState extends State<CartScreen> {
  ...
  Future<void> _loadData() async {
    final controller = Provider.of<CartController>(Get.context!, listen: false);
    await controller.getCartData(Get.context!);
    final uncheckedIds = controller.cartList
        .where((item) => item.isChecked != true && item.id != null)
        .map((item) => item.id!)
        .toList();
    if (uncheckedIds.isNotEmpty) {
      await controller.addRemoveCartSelectedItem(uncheckedIds, true);
    }
                                                                          ...
                                                                          children: [
                                                                            Row(children: [
                                                                              Container(
                                                                                ...
                                                                                child: Icon(Icons.shopping_bag_outlined, size: 19, color: Theme.of(context).primaryColor),
                                                                              ),
                                                                              Flexible(
                                                                                child: InkWell(child: Text(getTranslated('products', context) ?? '', maxLines: 1, overflow: TextOverflow.ellipsis, textAlign: TextAlign.start, style: textBold.copyWith(fontWeight: FontWeight.w500, fontSize: Dimensions.fontSizeLarge, color: Provider.of<ThemeController>(context, listen: false).darkTheme ? Theme.of(context).hintColor : Theme.of(context).textTheme.bodyLarge?.color))),
                                                                              ),
