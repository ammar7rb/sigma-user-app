...
class CustomerOrderInsuranceScreen extends StatefulWidget {
  ...
  @override
  State<CustomerOrderInsuranceScreen> createState() =>
      _CustomerOrderInsuranceScreenState();
}
    ...
    return Scaffold(
      ...
      body: Consumer<CustomerOrderInsuranceController>(
        ...
        if (controller.envelope == null) {
          return _ErrorState(
            message: controller.error?.toString() ??
                getTranslated('unable_to_load_insurance_claim', context)!,
        ...
        return RefreshIndicator(
          ...
          child: _ClaimContent(
              orderId: widget.orderId, envelope: controller.envelope!),
        );
      }),
    );
  }
...
class _ClaimContent extends StatelessWidget {
  ...
  Future<void> _offlineDialog(
    ...
    if (transferMethods.isEmpty) {
      showCustomSnackBarWidget(
          getTranslated('payment_method_not_properly_configured', context),
          context,
          snackBarType: SnackBarType.error);
      return;
    }
    String methodId = transferMethods.any((item) => item.id == initialMethodId)
        ? initialMethodId
        : transferMethods.first.id;
    XFile? proof;
    bool sending = false;
    String? error;
    final note = TextEditingController();
    final senderName = TextEditingController();
    final senderIdentifier = TextEditingController();
    await showDialog(
        context: context,
                ...
                actions: [
                  ...
                  ElevatedButton(
                    onPressed: proof == null ||
                            senderName.text.trim().isEmpty ||
                            senderIdentifier.text.trim().isEmpty ||
                            sending
                        ? null
                        : () async {
                            setState(() {
                            ...
                            final ok = await controller.submitOffline(
                              ...
                              senderIdentifier: senderIdentifier.text.trim(),
                            );
        ...
        builder: (dialogContext) => AlertDialog(
              ...
              content: Column(mainAxisSize: MainAxisSize.min, children: [
                ...
                TextField(
                    ...
                    maxLines: 3,
                    decoration: InputDecoration(
                        labelText: getTranslated('reason', context))),
              ]),
              actions: [
                TextButton(
                    onPressed: () => Navigator.pop(dialogContext),
                ...
                ElevatedButton(
                    onPressed: () async {
                      ...
                      final ok =
                          await controller.decline(orderId, reason.text.trim());
  ...
  void _showResult(
      BuildContext context, CustomerOrderInsuranceController controller) {
    if (controller.error != null) {
      showCustomSnackBarWidget(controller.error.toString(), context,
          snackBarType: SnackBarType.error);
    } else {
      showCustomSnackBarWidget(
          getTranslated('operation_completed_successfully', context), context,
